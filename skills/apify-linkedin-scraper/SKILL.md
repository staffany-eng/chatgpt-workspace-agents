...
Use this standalone skill to collect public LinkedIn data with Apify MCP. This is not tied to a Hermes bot packet.

## Preconditions

- Use only publicly visible LinkedIn pages, profiles, company pages, posts, feeds, articles, and engagement data.
- Require an Apify account and configured Apify MCP server. Use an `APIFY_TOKEN` only as the MCP bearer token or as a fallback for manual API debugging.
...
4. Call the actor only after confirming pricing and no-cookie/public-data behavior.
