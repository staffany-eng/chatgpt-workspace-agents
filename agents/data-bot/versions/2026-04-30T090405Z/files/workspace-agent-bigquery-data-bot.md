# Workspace Agent BigQuery Da Ta Bot

This document is the semantic layer to upload into the `Da Ta Bot` ChatGPT workspace agent.
...
| Revenue / usage health | `fct_company_org_mrr` | Company or linked-organization revenue/usage status, after schema inspection | Red-account and customer usage status candidate questions | Discover `company_usage_status`, `company_usage_next_step`, and related values before filtering. |
| Customer profile | `dim_companies` | One row per company, after schema inspection | Customer segment and industry candidate questions | Discover actual industry/segment fields and values before filtering for fitness customers. |

## Metric Registry Defaults

Upload `staffany-data-bot-metric-registry.md` alongside this semantic file and check it before answering these POC metrics:
...
Default status for all five POC definitions is candidate until a source/dashboard owner confirms the exact metric contract. Candidate answers are allowed when BigQuery can be queried safely, but the final response must use `confidence: needs-check` and include the caveat from the registry.

Do not treat `id_pph21_method = NETTO` as the definition of PPH on us. It is only a candidate signal.
...
- Default answer contract: answer, source table(s), filters/time window, confidence, and caveat.
- Confidence values must be exactly `verified`, `needs-check`, or `blocked`.
- Hide SQL unless asked.
- If BigQuery auth/tooling fails, return `confidence: blocked` and state the connector issue.
...
Additional POC regression prompts:
...
Each answer should show source table(s), filters/time window, confidence, and caveat. Candidate metrics should use `confidence: needs-check`; connector failures should use `confidence: blocked`.
