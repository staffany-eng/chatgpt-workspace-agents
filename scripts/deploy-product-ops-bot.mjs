...
log(`Preparing Product Ops Bot deploy from ${args.ref}`);
...
run(npmCommand, ["run", "product-ops-bot:verify"]);

const archiveName = `product-ops-bot-${deploySha}.tar.gz`;
const shaName = `product-ops-bot-${deploySha}.sha`;
...
if (!args.apply) {
  ...
  log(`Use \`npm run product-ops-bot:deploy -- --apply --ref ${args.ref}\` to deploy this exact ${args.ref} SHA.`);
  printSummary({
    deploySha,
    deployRef: args.ref,
    timestamp: "",
...
if (!args.skipUpload) {
  const scpTarget = `${sshUser}@${args.vm}`;
  run(gcloudCommand, [
    "compute",
    "scp",
    archivePath,
    `${scpTarget}:/tmp/${archiveName}`,
    "--project",
    args.project,
    "--zone",
    args.zone,
...
function parseArgs(argv) {
  ...
  const valueOptions = new Set(["project", "zone", "vm", "profile", "runtime-owner", "ssh-user", "ref"]);
...
function printHelp() {
  console.log(`Usage: node scripts/deploy-product-ops-bot.mjs [options]

Deploys an exact git ref for Product Ops Bot to the cloud Hermes profile.
...
archive="/tmp/product-ops-bot-$deploy_sha_expected.tar.gz"
sha_file="/tmp/product-ops-bot-$deploy_sha_expected.sha"
...
deploy_dir=$(mktemp -d /tmp/product-ops-bot-main.XXXXXX)
...
node scripts/verify-product-ops-bot.mjs
...
copy_dir "$deploy_dir/apps/product-ops-bot" "$profile/source/product-ops-bot"
copy_file "$deploy_dir/apps/product-ops-bot/profile/SOUL.md" "$profile/SOUL.md" 0644
copy_dir "$deploy_dir/apps/product-ops-bot/skills/product-ops-bot" "$profile/skills/product-ops-bot"
copy_dir "$deploy_dir/apps/product-ops-bot/skills/product-ops-intake-linking" "$profile/skills/product-ops-intake-linking"
copy_dir "$deploy_dir/apps/product-ops-bot/skills/staffany-product-delivery-workflow" "$profile/skills/staffany-product-delivery-workflow"
copy_dir "$deploy_dir/apps/product-ops-bot/runtime/mcp" "$profile/runtime/mcp"

python3 - "$deploy_dir/apps/product-ops-bot/profile/config.template.yaml" "$profile/config.yaml" "$runtime_owner" <<'PY'
import sys
...
copy_file "$deploy_dir/apps/product-ops-bot/runtime/check-health.sh" "$profile/scripts/productopsbot-check-health.sh" 0755
copy_file "$deploy_dir/apps/product-ops-bot/runtime/audit-live-profile.sh" "$profile/scripts/productopsbot-audit-live-profile.sh" 0755
...
sleep "\${PRODUCT_OPS_DEPLOY_GATEWAY_SETTLE_SECONDS:-15}"

run_post_deploy_check() {
  label="$1"
  shift
  attempts="\${PRODUCT_OPS_DEPLOY_CHECK_ATTEMPTS:-4}"
  delay_seconds="\${PRODUCT_OPS_DEPLOY_CHECK_RETRY_SECONDS:-10}"
  attempt=1
...
function log(message) {
  console.log(`[product-ops-bot-deploy] ${message}`);
}
