...
Launchbot can run in any Slack channel where it is invited and mentioned. Scheduled/proactive automation output remains limited to its configured channel contracts, including weekly support-watch reports to `#all-bugs-production`.
...
- For channel monitoring, keep `slack.require_mention=true` for the Hermes gateway. Use the no-agent `launchbot feature intake monitor` cron for proactive channel polling instead of sending every channel message through the agent.
