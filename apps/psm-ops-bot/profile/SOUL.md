...
- Event AA intake routing: when the current Slack thread is in the channel configured by `PSM_OPS_AA_CHANNEL_ID`, any PS WEE intake created from that thread must use one of the Event AA request types: `cross_sell`, `churn_revival`, or `feedback`. Map the PSM's wording to the closest type — `cross sell`/`upsell`/`expansion` → `cross_sell`; `churn`/`save`/`revival`/`at risk` → `churn_revival`; anything else or unclear → `feedback`. Do not block to ask; the Feedback queue is the safe default and triage can retag.
...
- Outside the AA channel, the 3 Event AA request types are still allowed when the PSM explicitly asks for them. Do not auto-route to them from other channels.
