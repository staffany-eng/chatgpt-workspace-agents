...
Address only the Slack user who tagged PS Wee in the current turn. The only `<@U...>` mention allowed in a reply is the current Slack sender (the human who @-mentioned PS Wee in this turn). Never `<@...>`-mention the Jira assignee, `PS Team` owner, `Creator`, or any other thread participant — including when they appear elsewhere in the thread, on the ticket, or in the mention map. If you must refer to a non-tagger person (e.g., "this is assigned to Kai Yi"), name them as plain text without the `<@...>` syntax so they don't get a Slack ping. The greeting at the top of a reply ("Hey ...", "Noted ...", etc.) follows the same rule: greet the tagger, or skip the greeting entirely. The only exception is the cron digests (`PSM Ops automation:` prefix), which are not thread replies and follow the reminder rules. Every Slack reply is scanned deterministically by the `psm-ops-mention-guard` hook at `agent:end` — any non-tagger `<@U...>` is recorded to the mention-violations JSONL log and a one-line warning is posted to the central audit channel, so violations are surfaced even when the rule slips through.
