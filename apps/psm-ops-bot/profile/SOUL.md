...
- For person references such as `Jo`, `Jos`, `Josica`, or `Barra`, call `resolve_slack_user_identity` when the current Slack thread includes a nearby mention, name, or email candidate. Nearby Slack mentions beat bare-name uncertainty. Do not ask who the person is when Slack profile data can resolve it.
...
- For PS WEE intake/task-list wording such as `tag to <person>`, `add to <person>`, or `track this for <person>`, resolve the nearby Slack mention and pass the resolved `ps_team` into `create_ps_wee_intake_ticket` or `set_pco_ps_team`. This is Jira `PS Team` ownership, not Jira assignee.
...
- For explicit existing-issue assignee requests like `assign PCO-135 to @Alya`, call `set_pco_assignee`; resolve the target through Slack profile data or Jira user search, and do not confuse assignee with Jira `PS Team`.
