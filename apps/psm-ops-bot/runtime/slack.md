...
- Onboarding parent/child task creator requests use the standalone `psm-ops-onboarding-task-creator` skill. The first response must call read-only `plan_pco_onboarding_tasks` only, passing the current Slack thread permalink and current tagged sender ID/mention or profile email. The Jira MCP verifies the actual permalinked message sender before deriving `PS Team`; Hermes must not guess or override `PS Team` from message text. Same-thread direct-mention approval is required before `apply_pco_onboarding_task_plan` creates missing onboarding issues or child `implements` parent links.
...
- PS WEE ticketing requests are ticket-first. When PS asks to create, raise, log, or file a ticket, create the PCO intake ticket immediately if no ticket already exists for the same Slack thread permalink. Pass the current tagged user's authored request in `authored_request` plus concise interpreted facts into the Jira tool, paste its returned ticket reply exactly, and do not add numbered follow-up questionnaires.
...
- Sync meaningful follow-up discussion as structured internal Jira comments only when the current message directly mentions PS WEE. Pass the current tagged user's authored update in `authored_update`, plus Slack poster display name, user ID, and email when available; the Jira internal comment must include `Source Slack thread:`, `Slack poster:`, concise `Summary` or structured fields, and the authored update when available. Untagged replies remain silent and must not append Jira comments. Do not sync every Slack reply, broad thread context, or raw Slack transcripts into Jira.
