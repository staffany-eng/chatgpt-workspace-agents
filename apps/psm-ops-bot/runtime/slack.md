...
- Onboarding parent/child task creator requests use the standalone `psm-ops-onboarding-task-creator` skill. The first response must call read-only `plan_pco_onboarding_tasks` only; same-thread direct-mention approval is required before `apply_pco_onboarding_task_plan` creates missing onboarding issues or child `implements` parent links.
