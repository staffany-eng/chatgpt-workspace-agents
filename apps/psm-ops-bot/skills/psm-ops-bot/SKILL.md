...
- Event AA label: every AA ticket is tagged with the Jira label `AA SG 2026` post-create. No agent action required.
...
- Event AA selfie ingest: for Event AA intakes only, the MCP fetches `image/*` files attached to the trigger Slack message via `conversations.history` (`SLACK_BOT_TOKEN` auth) and uploads each to the configured Google Drive folder (`PSM_OPS_AA_SELFIE_DRIVE_FOLDER_ID`) with filename `{company}_{pic}.{ext}`. Selfies are **not** attached to the Jira ticket for AA. Non-image files are skipped. Failures are best-effort and never block ticket creation; the resulting Slack reply mentions the saved count when at least one selfie was uploaded. When Drive is not configured, the upload is a silent no-op.
