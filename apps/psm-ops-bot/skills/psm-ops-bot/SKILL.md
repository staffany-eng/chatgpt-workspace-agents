...
- Resolve a single Slack mention, email, or exact name to safe identity fields and PS Team when available before asking avoidable owner questions.
...
- For person references such as `Jo`, `Jos`, `Josica`, or `Barra`, call `resolve_slack_user_identity` when the current thread includes a nearby Slack mention, name, or email candidate. Nearby Slack mentions beat bare-name uncertainty. Do not ask who the person is when the bot token can resolve the Slack identity.
...
- PS WEE ticket-intake requests are the only creation exception: the user's explicit ask to create, raise, log, or file a ticket is approval to create an intake ticket first. Call `find_ticket_by_slack_thread`, then `create_ps_wee_intake_ticket` if no same-thread ticket exists. Pass known customer, issue, impact, affected scope, expected outcome, and evidence facts into the tool so it can ask only the next missing fields.
...
- PS WEE intake/task-list phrases like `tag to <person>`, `add to <person>`, or `track this for <person>` mean Jira `PS Team` ownership when the resolved Slack user maps to a PS Team option. Pass the resolved `ps_team` into `create_ps_wee_intake_ticket` or `set_pco_ps_team`; do not use Jira assignee for this wording.
...
- For explicit existing-issue assignee requests like `assign PCO-135 to @Alya`, call `set_pco_assignee`. Assignee updates are Jira person assignment; `PS Team` remains the source of truth for "my tasks" and reminders.
