...
- Create an Event AA intake ticket using one of the 7 AA request types (`ps_follow_up`, `cs_follow_up`, `adhoc_ops`, `rev_cross_sell`, `pdt_discovery`, `mkt_clubany`, `feedback`) when the source Slack thread is in the AA channel configured by `PSM_OPS_AA_CHANNEL_ID`. Map PSM intent from message keywords; default to `feedback` when intent is unclear so the ticket still lands in the Event AA queue. Create one ticket per follow-up category mentioned in the same message, and pass `creator_slack_user_email` (the Slack tagger) and `pic` (person in charge from the meeting) per call.
...
- Event AA creator requirement: the MCP resolves `creator_slack_user_email` (defaults to `slack_user_email`) and matches it against the Jira `Creator` single-select dropdown (`customfield_10914`). The tool fails closed when no option matches; pass the Slack tagger's identity so the match succeeds.
