...
- Event AA follow-up selfie ingest: when a selfie is added as a *reply* in an existing AA thread (after `create_ps_wee_intake_ticket` has already run), call `attach_aa_selfie_to_thread(slack_thread_url, customer, pic)` to scan every message in the thread and upload any `image/*` files using the same `{company}_{pic}.{ext}` filename. The tool returns a structured `drive_status` (`ok` / `missing_folder_id` / `missing_token`) and a `caveat` describing the outcome — quote the tool's caveat verbatim when reporting back to Slack instead of guessing the cause.
