...
- Customer 360 currently exposes MRR only for this compact revenue lookup. For ARR, deal value, renewal amount, billing amount, or other non-MRR revenue asks, do not reuse `answer.summary.totalMrr`; return a bounded `needs-check` or unavailable response unless the request is reframed as MRR.
