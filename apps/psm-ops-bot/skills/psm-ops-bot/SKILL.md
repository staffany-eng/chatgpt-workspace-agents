...
- For MRR, ARR, deal value, revenue snapshot, renewal amount, or billing amount questions, treat the requested value as compact account data, not compiled wiki knowledge. Resolve the customer if needed, then call `get_c360_account_context(customer_key, format="json")` and answer from `answer.summary.totalMrr` when present. Do not call `ask_c360_customer_context` first for MRR, and do not say MRR is unavailable just because the compiled wiki did not cite it.
