...
- For Event AA intakes only, the MCP fetches `image/*` files attached to the trigger Slack message and uploads them to the configured Google Drive folder (`PSM_OPS_AA_SELFIE_DRIVE_FOLDER_ID`) with filename `{slugified_company}_{slugified_pic}__{slack_file_id}{ext}`. Selfies are **not** attached to the Jira ticket. Non-image files are skipped. The download+upload is best-effort: per-file errors are silently dropped, and the Slack reply mentions the saved count when at least one selfie was uploaded. When the Drive folder or OAuth files are not configured, the trigger-message upload is a silent no-op (saved-count line is omitted; the agent must not name an environment variable as the cause). Implemented by `_download_slack_images_for_drive` + `upload_aa_selfies` (`aa_selfie_drive.py`).
- For follow-up selfies posted as a *reply* in an existing AA thread (after `create_ps_wee_intake_ticket` has already created the per-category tickets), call `attach_aa_selfie_to_thread(slack_thread_url, customer, pic)`. The tool scans the whole thread via `conversations.replies`, downloads any `image/*` files, and uploads them via `upload_aa_selfies` using the same filename convention. It returns a structured `drive_status` (`ok` / `missing_folder_id` / `missing_token`), `saved_count`, `already_present_count`, and a human-readable `caveat`. `aa_selfie_drive.configuration_status()` is the single source of truth for the status code (`missing_folder_id` is presently unreachable because the live folder is the default, but the code remains in the surface so future env wiring can flip it on without a contract change).
- Idempotency: every Drive upload from `upload_aa_selfies` writes `appProperties.slack_file_id = <Slack file id>` on the created Drive file. Before uploading, the function lists the AA folder for any file already tagged with the same `slack_file_id` (scope-compatible with `drive.file`, which only sees files this app created) and returns that existing record with `already_present=true` instead of re-uploading. This makes `attach_aa_selfie_to_thread` safe to call multiple times on the same thread, and lets `create_ps_wee_intake_ticket` reruns avoid duplicating trigger-message selfies as long as the Drive record was created by this app. Pre-existing files in the folder that were not uploaded by this app are invisible to the dedupe query and would still be re-uploaded if Slack happens to re-attach them, but the folder is currently empty of bot uploads so this is not a live concern.
- Filename slugify rules (`aa_selfie_drive._slugify`): runs of non-alphanumeric ASCII characters (whitespace, punctuation, Unicode) are replaced with a single `-`, leading/trailing dashes are stripped, and the result is lowercased. Empty results fall back to `unknown`. Example: `Kopi Janji (SG) Pte Ltd` → `kopi-janji-sg-pte-ltd`. The extension comes from the original Slack filename when present, otherwise from the mime-type map (`image/jpeg`→`.jpg`, `image/png`→`.png`, etc.; fallback `.jpg`). The Slack file id is suffixed (`__{slack_file_id}`) before the extension so distinct selfies for the same `(company, pic)` never share a filename — re-uploads of the same Slack file produce the same Drive name, which together with the `appProperties.slack_file_id` dedupe makes the upload idempotent. If `slack_file_id` is missing (callers outside the Slack ingest path), the function falls back to a `-{n}` numeric suffix on the second and later files in the same call. No length capping is applied — Drive accepts long names.
