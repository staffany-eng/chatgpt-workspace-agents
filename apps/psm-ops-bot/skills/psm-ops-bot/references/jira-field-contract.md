...
- Company name lands in **both** the text customer field and the StaffAny Organisation object field (`PSM_OPS_JIRA_FIELD_STAFFANY_ORGS`; thin POC default `customfield_10667`) when the company resolves through the reviewed customer-channel map. `customfield_10667` is a Jira Assets object reference, so the MCP always sends it as an **array of strings** (asset keys / object names). When the channel-map lookup misses, the MCP falls back to `[supplied_customer]` as a best-effort try — if the customer name happens to match an existing asset's name it lands; otherwise Jira rejects the field value and the create retries with the field omitted (the ticket still lands, just without an org assigned). Agents should prefer to call `search_c360_customers` first and pass the canonical `orgMatches[].matchedValue` as `staffany_orgs` to maximize the asset match rate.
