...
- Target-account news-scout requests must resolve the account through the caller's NurtureAny HubSpot scope before public research. After `run`, use `target-account-news-scout` for recent public signals and return a manual-review draft only; do not auto-send outreach.
- Target-account news-scout first responses must use the standard five-line preflight only. Do not add checklists, prerequisites, or extra headings before `run`.
