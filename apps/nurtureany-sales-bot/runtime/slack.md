...
- SG/MY WhatsApp morning report prompts should plan `build_sales_whatsapp_window_report` for one-off windows and `save_sales_whatsapp_window_report_schedule` only when the user explicitly asks to save/update the weekday schedule. Default report contract: Singapore then Malaysia, 09:30-10:30 owner-local time, target 30, `include_kns=false`, owner/country/timezone from the runtime access policy through `resolve_sales_owners`, no Slack-memory roster fixes, and no silent SGT fallback. Final answers must render `answer.slack_markdown` or summarize only returned `answer.country_rows` for `answer.countries`; do not add Indonesia/admin-expansion/timezone-gap notes unless the primitive returned those rows. A prompt like `9:45-10:45 today` is an ad hoc rerun and must not mutate schedule state.
