...
- After `run`, or for a clear bounded same-thread continuation after a delivered result, `get_current_slack_thread_context` and `get_selected_slack_thread_context` may read one configured-channel Slack thread, capped at 50 messages, and return safe summaries/permalinks only. These tools must not be used for broad channel history, workspace search, user listing, reactions, pins, arbitrary posting, raw transcript export, or any user-token fallback.
- Slack capability answers must be precise: NurtureAny can read Hermes-injected current thread context and can call bounded bot-token Slack read tools for configured channels; it cannot browse arbitrary Slack history, search all channels, list users broadly, pin/react, or post arbitrary Slack messages through exposed tools.
...
Explicit selected-thread reads after `run` or bounded continuation also use bot-token `conversations.replies` and `chat.getPermalink` for configured channels only through `get_current_slack_thread_context` or `get_selected_slack_thread_context`, capped at 50 messages with safe summaries/permalinks only.
