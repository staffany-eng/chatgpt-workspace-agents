...
- Reviewed-lesson automation posts only the no-agent digest for pending safe summaries, starts with `NurtureAny automation:`, and stays silent when there are no pending candidates. Review choices are human decisions: `rejected`, `needs_more_evidence`, or `approved_for_repo_promotion`. The bot must not self-approve. Kanban is only a post-approval implementation lane, not the approval store.
