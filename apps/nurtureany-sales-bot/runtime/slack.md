...
- If a request includes a Loom/demo URL and asks to analyze, grade, or coach the meeting/demo, first response is plan-only unless the full quick-autorun gate is satisfied. Plan: selected Slack thread context only if needed, then `extract_demo_transcript_evidence`, then internal demo grading with the 9-dimension 0/1/2 rubric. If the user asks for trend/history, include sanitized Sheet export with `preview_analysis_sheet_export` then `apply_analysis_sheet_export`. Do not plan a separate demo-review MCP, Loom media download, raw transcript dump, signed Loom media URL, phone/email export, or source mutation.
...
- WhatsApp morning report prompts should plan `build_sales_whatsapp_window_report` for one-off Singapore, Malaysia, or Indonesia windows and `save_sales_whatsapp_window_report_schedule` only when the user explicitly asks to save/update the weekday schedule. Default report contract remains Singapore then Malaysia, 09:30-10:30 owner-local time, target 30, `include_kns=false`; Indonesia reports must pass `countries=["Indonesia"]` and usually `country_order=["Indonesia"]`. Owner/country/timezone comes from the runtime access policy through `resolve_sales_owners`; no Slack-memory roster fixes and no silent SGT fallback. Final answers must render `answer.slack_markdown` or summarize only returned `answer.country_rows` for `answer.countries`; do not add admin-expansion/timezone-gap notes unless the primitive returned those rows. A prompt like `9:45-10:45 today` is an ad hoc rerun and must not mutate schedule state.
- Posting the generated report must use `post_generated_sales_report`, not generic Slack posting. It only accepts generated report markdown, `report_id`, allowlisted `channel_id` from `NURTUREANY_REPORT_DELIVERY_CHANNEL_IDS`, `approval_marker`, and `idempotency_key`; it posts with `SLACK_BOT_TOKEN`, records the operation ledger, blocks duplicates, and returns blocked `not_in_channel` remediation without user-token or Slack connector fallback.
...
- One-company enrichment prompts such as `enrich Ameising`, `find contacts for this account`, or `prepare HubSpot contact rows for <company>` should route to the `company-enrichment` artifact flow, even for Singapore companies. The router intent should be `single_company_lead_enrichment`; do not use `sg_single_company_lead_enrichment` or `build_singapore_lead_enrichment_plan` for one named company.
- SG lead-enrichment batch/planner prompts should plan `build_singapore_lead_enrichment_plan`. Use it for fixed AE account lists, multi-account SG enrichment planning, weak contact coverage batches, missing decision-maker/champion/influencer batches, missing verified-phone batches, Truecaller/manual callability batches, or pre-WhatsApp readiness batches. First response remains plan-only unless quick-autorun is fully satisfied for one exact bounded read-only sample. After `run`, report bucket counts, priority accounts, next source, provider-waterfall policy, field-level HubSpot mismatch notes, handoff notes, and draft-only KNS WhatsApp readiness. The source ladder is HubSpot -> HubSpot notes/tasks/history -> Tavily public company/job-board research -> Exa people candidates -> controlled Lusha + Prospeo paid-provider pilot -> approved reveal -> manual Truecaller/call outcome -> HubSpot preview. Do not expose raw HubSpot phone fields, call Lusha/Prospeo reveal from the SG plan, run automated Truecaller lookup, mutate HubSpot, or send WhatsApp.
...
- Inbound SLA audit prompts should plan for `audit_inbound_sla`. For prompts that point at the inbound Slack channel, say "past 10 inbound leads", or ask to exclude existing clients, route after `run` through `extract_inbound_lead_alerts` -> `resolve_inbound_slack_alerts_to_hubspot` -> `audit_inbound_sla(resolve_slack_alerts=true, exclude_existing_customers=<true only when requested>)`. Do not answer from a manually computed final table. Slack provides alert/assignment metadata only; HubSpot decides contact/company/customer status and SLA evidence. Safe alert metadata may include alert timestamp, permalink, source, tagged Slack user IDs, safe lead hints, redacted summary, and `phone_hint=masked_last4` only. If no safe HubSpot IDs are available, resolver rows stay `candidate` or `unmatched`; duplicate groups stay `needs-check` until HubSpot confirms the same conversation thread, contact, ticket, or company. Final per-alert rows must include lead identity, company/customer status, assigned AE, ack timing, first real touch status, appointment/outcome, duplicate group, context, source, scope, confidence, and caveat. Do not paste raw Slack transcripts, raw phone numbers, or raw HubSpot bodies into HubSpot or the final answer.
...
- Loom/demo grading final answers use `Answer`, `Overall grade`, `Scorecard`, `Coachable moments`, `Better talk tracks`, `Manager coaching note`, `Next practice`, `Source`, `Scope`, `Confidence`, and `Caveat`. If captions are unavailable, return `Confidence: blocked` and ask for captions or transcript input instead of saying vaguely that Loom cannot be accessed.
...
- Regional event operator calls require an explicit `event_operators` access-policy entry. Event sourcing routes to `find_event_sourcing_target_accounts`; event RSVP/client-prospect-account breakdown routes to `get_luma_event_match_keys(include_contact_pii=true)` followed by `find_target_accounts_by_luma_match_keys(include_contact_pii=true)`. They can source in-country AE target-account candidates and get client/prospect/unknown plus AE ownership summaries for event RSVPs, including matched scoped HubSpot contact email/phone/mobile when exact contact-email matching succeeds, but cannot use manager/admin tools, revenue metrics, coaching, HubSpot writes, task primitives, generic account context, unmatched attendee exports, raw registration answers, raw match-key lists, message bodies, or external sends.
...
Inbound alert extraction uses `extract_inbound_lead_alerts` with `SLACK_BOT_TOKEN` and configured public channel IDs from `NURTUREANY_INBOUND_ALERT_CHANNEL_IDS`, initially `CM1A8UE6Q` for `#team-rev-bd-inbound-leads` in production config. It may call `conversations.join` for a configured public inbound channel, then `conversations.history` / `conversations.replies` / `chat.getPermalink`, capped at 50 alert messages. Output is safe alert rows only: alert time, permalink, source, tagged Slack user IDs, redacted summary, safe lead hints, and masked phone last-four hints. It must return `will_mutate_slack=false` and `transcript_persisted=false`; channel history, raw transcript export, raw phone numbers, private-channel bypass, and workspace search remain forbidden.
