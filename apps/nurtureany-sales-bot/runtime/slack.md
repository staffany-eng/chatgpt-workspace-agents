...
- Daily nurture automation is disabled pending refinement. Do not install a scheduled 09:00 daily nurture cron, scheduled noon daily nurture reminder cron, or advertise a ready Jeremy daily-pack workflow. HubSpot Task reminder digests are separate no-agent automation that read incomplete HubSpot Tasks by `hs_timestamp` until `hs_task_status=COMPLETED`.
