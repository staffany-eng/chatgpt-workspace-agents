...
- Aircall coaching smoke check uses `analyze_aircall_call_coaching` for one selected numeric call ID, defaults to OpenAI `gpt-4o-transcribe-diarize` plus `gpt-5.5`, computes transcript/timing interaction metrics locally, supports `call_type="demo"` with the 9 demo dimensions, returns safe coaching JSON only, says audio-native tone was not checked for call reviews, and never returns raw transcript/audio/recording URLs/phone numbers or claims Gong/ElevenLabs integration.
- Demo Sources MCP lists only `extract_demo_transcript_evidence` when demo sources are enabled.
- Demo Sources smoke check fetches one selected Loom share page, extracts captions/VTT internally, returns title, cue count, word count, bounded redacted segments, timing metadata, canonical permalink, confidence, and blocker reason, and never returns raw transcript dumps, signed Loom media URLs, video/audio bytes, phone numbers, full emails, or source mutations. If captions are unavailable, private, blocked, or malformed, it returns `Confidence: blocked` and asks for captions or transcript input.
...
- HubSpot single-company enrichment smoke check confirms `resolve_company_enrichment_target` blocks ambiguous or out-of-scope names, `create_company_enrichment_artifact` starts only after one scoped SG/MY/ID company is selected, `update_company_enrichment_artifact` appends level evidence or raw provider `tool_result` for Tavily read-through, Exa read-through, standalone public people/job-board search, Lusha, and Prospeo, default artifact reads redact revealed PII, `summarize_company_enrichment_artifact` returns `will_mutate_hubspot=false` plus `waterfall_state`, full-waterfall claims are blocked until `can_claim_full_waterfall=true`, and Lusha/Prospeo reveal remains manually approval gated.
...
- Apify MCP lists only `search-actors`, `fetch-actor-details`, `call-actor`, `get-actor-run`, `get-actor-output`, `search-apify-docs`, and `fetch-apify-docs` when Apify is enabled. Apify requires `APIFY_TOKEN` from profile dotenv/Secret Manager, and social scraping is explicit-run, budget-confirmed, public-pages-only by default, and manual-review evidence only.
...
- Lusha MCP lists only `search_lusha_decision_maker_candidates`, `search_lusha_candidates_by_linkedin_urls`, `search_lusha_candidates_by_names`, `reveal_lusha_contact_details`, and `get_lusha_credit_usage` when Lusha is enabled.
