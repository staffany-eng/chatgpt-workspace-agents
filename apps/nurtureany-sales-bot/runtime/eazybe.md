...
- `NURTUREANY_DAILY_RUNS_DIR`: optional local JSON run payload directory shared with `build_daily_nurture_plan`, so 12pm preview/status/reminder tools can reload the 9am `run_id` messages.
...
- `build_daily_nurture_reminder`: builds the 12pm Slack reminder text for unsent and unskipped stakeholder messages.
