...
- Output: event metadata, RSVP/checked-in counts, safe email domains, and company-name candidates. For past events, keys come only from checked-in guests; if `checked_in_count=0`, do not match RSVP/no-show guests as attended.
- With `include_contact_pii=true`, output also includes bounded normalized `attendee_emails` as transient exact HubSpot contact-email match keys. This is for the internal Jan-E/event-operator account-breakdown flow only; do not paste raw key lists in Slack.
- Does not return attendee names, phone numbers, raw registration answers, or raw guest lists.
- Follow with `find_target_accounts_by_luma_match_keys`, which matches in this order: `exact_hubspot_contact_email` -> `exact_email_domain` -> `company_name_candidate`, adds HubSpot owner and customer/prospect/unknown status, and may return matched scoped HubSpot contact email/phone/mobile when `include_contact_pii=true`. Then call `get_luma_event_context` using only those scoped candidate companies when matched attendee context is needed. Explicit regional event operators may use this read-only match-key account-resolution step for their configured countries; they still cannot use generic account context or manager tools.
...
4. Build the Slack output from that one matched-account snapshot: customer/client vs prospect vs unknown/candidate counts, AE owner rollups, matched contact details for exact scoped contact-email matches, source, scope, confidence, and caveat.
