...
- `audit_inbound_sla`: read-only inbound operating audit. It combines recent HubSpot Conversations, safe CRM activity, and optional safe Slack alert metadata to return SLA rows, duplicate groups, HubSpot hygiene gaps, and the required thread response format. After `run`, an approved rerun, or a same-thread correction with clear scope, call this tool and use the returned rows as the answer source; do not manually compute the final audit table. No auto-reassign, HubSpot mutation, raw Slack transcript, or external message send.
...
- `build_daily_nurture_plan`: manager/admin daily nurture queue plan. It reads scoped HubSpot hygiene, follow-up, event, material, and Eazybe-readiness context, returns draft message rows and run metadata, and never sends WhatsApp or mutates HubSpot.
...
- `read_nurture_material_registry`: read-only Google Sheets lookup for the one-sheet NurtureAny material registry configured by `NURTUREANY_MATERIAL_REGISTRY_SPREADSHEET_ID`. It returns safe registry rows only and never returns phone numbers, full emails, raw exports, or Drive mutations.
...
- `preview_eazybe_template_messages`: approval-gated preview of selected daily nurture template messages; returns redacted phone metadata and requires stored daily run rows.
- `send_approved_eazybe_messages`: sends only selected approved-template Eazybe messages with an explicit approval marker; no free-form WhatsApp text.
- `check_eazybe_send_status`: read-only Eazybe delivery status check for selected message IDs.
- `build_daily_nurture_reminder`: preview-only Slack reminder text for daily nurture review, approval, and follow-up.
...
Public research, Exa, and Lusha search inputs must come from NurtureAny scoped HubSpot account output and include a HubSpot `company_id` plus `scope_source=hubspot_nurtureany` or `hubspot_scoped=true`. Do not use paid enrichment sources for arbitrary company-name-only inputs.
