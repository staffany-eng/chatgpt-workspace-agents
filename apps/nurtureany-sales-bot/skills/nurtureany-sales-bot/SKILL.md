...
- `list_marketing_campaigns`: manager/admin read-only HubSpot Marketing Campaigns lookup. Campaign metadata is campaign evidence only and does not prove engagement, QO, QO Met, closed-won, pipeline, or revenue attribution.
- `get_campaign_assets`: manager/admin bounded HubSpot campaign asset lookup. Form/page/email associations and asset metrics are context only; missing asset metrics or association evidence is not proof of no engagement.
