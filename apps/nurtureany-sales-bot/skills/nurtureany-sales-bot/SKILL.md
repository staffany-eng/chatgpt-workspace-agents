...
- `get_account_context`: one company with associated contacts, deals, activities, C360, Google Calendar, and Luma context. It must expose HubSpot owner name/email, customer/prospect status/source, `c360_url` for verified customer accounts, `account_packet` for default Slack account-background answers, contact coverage source fields, the recommended AE calendar ID for follow-up scans, and `calendar_audit_seed` with safe contact email hashes/domains for meeting-quality audit. For account-background answers, render `account_packet.slack_markdown` by default.
