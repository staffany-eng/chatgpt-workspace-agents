...
If Hermes exposes `get_current_slack_thread_context` or `get_selected_slack_thread_context`, call them only after `run` or for a clear bounded same-thread continuation. They may read one configured thread-context channel thread, capped at 50 messages, and must return safe summaries/permalinks only. They use the bot token and do not depend on Kai Yi being a channel member; if a configured public source channel returns `not_in_channel`, the adapter may call `conversations.join` with the bot token and retry. The channel must still be in `NURTUREANY_SLACK_THREAD_CONTEXT_CHANNEL_IDS` or the narrower intent-channel fallback. Do not use them for broad channel history, private-channel bypass, workspace search, user listing, Slack posting, reactions, pins, raw transcript export, user tokens, or Slack connector fallback.

If asked about Slack capabilities, say NurtureAny can read Hermes-injected current thread context and bounded bot-token thread context for configured thread-context channels. Do not say it has no Slack API access at all, and do not imply it can browse arbitrary Slack history or post arbitrary Slack messages.
