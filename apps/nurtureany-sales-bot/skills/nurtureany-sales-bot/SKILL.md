...
- Questions about QO, new ARR, revenue pace, or revenue snapshots when they are scoped to target-account nurture, AE queues, manager rollups, or Friday review. Direct QO count/pace prompts with an owner/team and date range are revenue-metric prompts even when they name an AE.
...
12. StaffAny C360 BigQuery tools for commercial value, renewal timing, MRR, account owner, PSM context, QO sales points, converted ARR, MRR movements, revenue snapshots, and current-customer product/Payroll truth for account packets.
...
- `get_luma_event_match_keys`: read-only Luma event-first matching helper. It returns safe company email domains and company-name candidates for HubSpot target-account lookup, and never returns attendee names, full emails, phone numbers, registration answers, or raw guest lists.
...
For revenue metric flows, name the metric definition, source class, and as-of period. For QO pace, use `fct_sales_points.qo_set` after schema inspection. If the prompt is a direct QO count or pace question, for example `what is Jeremy's QO in April` or `what's my QO this month`, do not plan `build_friday_sales_review`; plan owner/team scope resolution plus StaffAny BigQuery actuals. If the user says `new ARR`, ask whether they mean signed converted ARR, paid converted ARR, or new MRR movement annualized before running BigQuery.
...
27. Routing direct QO count/pace questions to `build_friday_sales_review` instead of the `fct_sales_points.qo_set` revenue-metric path.
