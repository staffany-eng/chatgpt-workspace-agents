...
For Indonesia LL/HHH flows where Luma check-in is empty or not used, use `read_indonesia_event_registration_attendance` as the viable fallback. Use the Sheet `Attend The Event` column only as manual attendance evidence, then resolve attended company/domain keys to scoped HubSpot target accounts before account-level output. Keep the caveat that Sheet fallback is manual and `Confidence: needs-check` until HubSpot scope and follow-up evidence are checked.
