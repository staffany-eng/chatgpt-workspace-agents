...
| `get_marketing_campaign_attribution` | Marketing | Searches bounded HubSpot contact source fields and maps only scoped contacts/companies before checking configured QO/QO Met/closed-won deal stages; no raw PII, raw form rows, generic QO-total substitution, or mutation. |
...
| `build_sales_metric_actuals_query` | Revenue actuals | Builds scoped aggregate SQL for QO and revenue actuals; Rev Sheets/Slides are targets/definitions only; execute only through `staffany_bigquery.execute_sql_readonly`; access scope, no raw deal/contact rows, no mutation. |
