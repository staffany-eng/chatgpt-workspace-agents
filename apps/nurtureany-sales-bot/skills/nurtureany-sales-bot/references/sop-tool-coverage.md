...
| `get_current_slack_thread_context` | Slack thread | Explicit selected-thread read after `run` or bounded continuation only; one configured thread-context channel, max 50 messages, bot token only, no Kai Yi channel membership dependency, may auto-join configured public source channels via `conversations.join`, safe summaries/permalinks only, no private-channel bypass, raw transcript export, broad history/search, user listing, posting, reactions, pins, user-token fallback, or mutation. |
| `get_selected_slack_thread_context` | Slack thread | User-supplied permalink read after `run` only; parse one configured thread-context channel, max 50 messages, bot token only, no Kai Yi channel membership dependency, may auto-join configured public source channels via `conversations.join`, safe summaries/permalinks only, no private-channel bypass, raw transcript export, broad history/search, user listing, posting, reactions, pins, user-token fallback, or mutation. |
