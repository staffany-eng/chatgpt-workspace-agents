...
| explicit `event_operators` policy entry | Regional event operator | Read-only in-country event sourcing through `find_event_sourcing_target_accounts` and Luma RSVP account resolution through `get_luma_event_match_keys(include_contact_pii=true)` plus `find_target_accounts_by_luma_match_keys(include_contact_pii=true)` |
...
Event-operator access is also explicit. Use it for regional events people who need safe target-account sourcing across in-country AE accounts or event RSVP client/prospect/owner breakdowns from Luma match keys. Exact scoped HubSpot contact-email matches may return matched contact email/phone/mobile in internal event output only. It does not grant manager/admin tools, revenue metrics, coaching audits, HubSpot Task primitives, generic write-back previews, broad account context, unmatched attendee exports, raw registration answers, raw match-key lists, message bodies, or external sends.
...
For exact local WhatsApp windows, rep timezone comes from `NURTUREANY_ACCESS_POLICY_PATH` or explicit `timezone_override_by_owner_email`. Slack mentions and HubSpot owner records resolve identity only; they are not timezone sources. If timezone is missing or invalid, do not run a UTC timestamp query and do not return a misleading zero count.
...
| Object | Safe properties | Usage |
| --- | --- | --- |
| Communications | `hs_timestamp`, `hubspot_owner_id`, `hs_communication_channel_type`, `hs_communication_logged_from` | Count meaningful-activity touches only when `hs_communication_channel_type=WHATS_APP`. |
| Calls | `hs_timestamp`, `hubspot_owner_id`, `hs_call_title`, `hs_call_status`, `hs_call_direction`, `hs_call_duration`, `hs_lastmodifieddate` | Count meaningful-activity touches only when the call is completed, outbound, and not titled as a follow-up call. Count connected calls when status is completed and duration is at least 120 seconds; use the outbound connected count for Eugene's meaningful-activities report. |
...
Friday report output mirrors the tactical pause docs: 120/150 account coverage, double tap, 30 WhatsApp daily rhythm, 40 connected calls, QO/QO Met guardrail, warm activity proof, Friday correction, coaching observations, and next-week actions. For Eugene's meaningful-activities KPI, 2-touch coverage means total target accounts with 2+ meaningful activities out of 150; the hit threshold is 120/150. The protected pool is selected per owner from HubSpot target accounts, optional headcount filters, optional associated deal pipeline/stage filters, then deterministic top-150 priority ordering. Notes and tasks are not meaningful touches. If `NURTUREANY_QO_PIPELINE_IDS`, `NURTUREANY_QO_STAGE_IDS`, `NURTUREANY_QO_MET_STAGE_IDS`, or `NURTUREANY_CLOSED_WON_STAGE_IDS` is missing, the report still returns hygiene/account coverage with `Confidence: needs-check`.
