...
| Regional event operator | Explicit `event_operators` policy entry | Read-only in-country event sourcing through `find_event_sourcing_target_accounts` and Luma RSVP match-key account resolution through `find_target_accounts_by_luma_match_keys`; no manager/admin tools, HubSpot writes, coaching, revenue metrics, generic account context, raw attendee exports, or raw contact PII |
